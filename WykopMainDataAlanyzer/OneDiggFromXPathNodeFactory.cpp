...
OneDigg OneDiggFromXPathNodeFactory::createDigg(pugi::xpath_node &node) {
	...
	auto title = extractor.getValueFromNode(node, "title/text()");
	auto description = extractor.getValueFromNode(node, "description/text()");
	int diggsCount = getDiggsCount(node, extractor);

	std::string date = fetchDate(node, extractor);
	std::string dateWithTime = extractor.getValueFromNode(node, "time/text()");
...
int OneDiggFromXPathNodeFactory::getDiggsCount(const pugi::xpath_node &node, ValuesFromNodeExtractor &extractor) const {
	auto diggsCountAsString = extractor.getValueFromNode(node, "diggCount/text()");
	int diggsCount;
	try {
...
std::string OneDiggFromXPathNodeFactory::fetchDate(const pugi::xpath_node &node, ValuesFromNodeExtractor &extractor) const {
	auto dateWithHour = extractor.getValueFromNode(node, "time/text()");
	std::stringstream ss;
	ss << dateWithHour;
	std::string date;
	ss >> date;
