...
int wykopMainDataAnalyzerMain(int argc, const char **argv) {
	...
	std::vector<pugi::xpath_node> nodes = fetcher.fetchNodes(InputFileConstants::getPathToDiggNodes().c_str());
	...
	std::map< Characteristic, std::function<CharacteristicValue (OneDigg&)> > countingMethodByCharacteristicMap{
			{"DiggsOnMain", [](OneDigg& UNUSED){ UNUSED.getAddDate();// to suspend warnings
				return 1;}},
